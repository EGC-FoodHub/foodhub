...
from app import db
from .models import FoodDataset, DSDownloadRecord, DSViewRecord

class FoodTrendingService:
    
    @staticmethod
    def get_trending_by_downloads(timeframe='week', limit=5):
        """
        Obtiene datasets trending basado SOLO en descargas (como en el ejemplo visual)
        """
        if timeframe == 'week':
        ...
        trending = db.session.query(
            FoodDataset,
            func.coalesce(downloads_subq.c.download_count, 0).label('recent_downloads')
        ).outerjoin(
            downloads_subq, downloads_subq.c.dataset_id == FoodDataset.id
        ).filter(
            FoodDataset.kind == "food",
            func.coalesce(downloads_subq.c.download_count, 0) > 0
        ).order_by(
    ...
    @staticmethod
    def get_trending_by_views(timeframe='week', limit=5):
        """
        Obtiene datasets trending basado en vistas recientes
        """
        if timeframe == 'week':
            since_date = datetime.utcnow() - timedelta(days=7)
        else:  # month
        ...
        trending = db.session.query(
            FoodDataset,
            func.coalesce(views_subq.c.view_count, 0).label('recent_views')
        ).outerjoin(
            views_subq, views_subq.c.dataset_id == FoodDataset.id
        ).filter(
            FoodDataset.kind == "food",
            func.coalesce(views_subq.c.view_count, 0) > 0
        ).order_by(
            desc('recent_views')
        ).limit(limit).all()
    ...
    def get_popularity_ranking(limit=10, timeframe_days=7):
        """
        Ranking combinado usando un scoring system - VERSIÓN MEJORADA
        """
    ...
    def get_trending_for_homepage():
        """Método específico para la página principal - VERSIÓN MEJORADA"""
        return {
            'weekly_downloads': FoodTrendingService.get_trending_by_downloads(timeframe='week', limit=3),
            'weekly_views': FoodTrendingService.get_trending_by_views(timeframe='week', limit=3),
    ...
    @staticmethod
    def get_simple_trending_display(timeframe='week', limit=3):
        """
        Versión simplificada específica para mostrar como en el ejemplo visual
        Retorna: lista de strings formateados "Título - X downloads"
        """
        if timeframe == 'week':
        ...
        display_list = []
        for dataset, downloads in trending_data:
            display_list.append(f'"{dataset.title}" - {downloads} downloads')
