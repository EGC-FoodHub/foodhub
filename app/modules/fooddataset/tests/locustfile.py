...
class TrendingLoadTestBehavior(TaskSet):
    ...
    def get_statistics(self):
        ...
        with self.client.get(
            "/api/statistics",  # Ruta de ejemplo
            name="API - Statistics",
            catch_response=True
        ) as response:
            if response.status_code == 200:
    ...
    def list_datasets(self):
        ...
        with self.client.get(
            "/datasets",  # Ruta de ejemplo
            name="Página - List datasets",
            catch_response=True
        ) as response:
            if response.status_code == 200:
    ...
    def view_dataset_detail(self):
        ...
        if self.dataset_ids or self.last_trending_data:
            ...
            if dataset_id:
                with self.client.get(
                    f"/dataset/{dataset_id}",  # Ruta de ejemplo
                    name="Página - Dataset detail",
                    catch_response=True
                ) as response:
                    if response.status_code == 200:
                        response.success()
    ...
    def simulate_user_session(self):
        ...
        self.client.get("/api/datasets/trending", 
                       params={"days": 7, "limit": 10},
                       name="Flujo - Trending API")
        self.wait()
        ...
        if self.last_trending_data:
            ...
            dataset_id = dataset.get('id')
            if dataset_id:
                self.client.post(f"/api/dataset/{dataset_id}/view", 
                                json={"timestamp": datetime.now().isoformat()},
