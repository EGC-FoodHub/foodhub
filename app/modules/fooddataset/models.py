...
class FoodDataset(BaseDataset):
    ...
    def get_recent_views(self, days=7):
        ...
        return db.session.query(func.count(FoodDatasetActivity.id)).filter(
            and_(
    ...
    def get_trending(period_days=7, limit=10):
        ...
        downloads_subquery = db.session.query(
        ...
        trending_datasets = db.session.query(FoodDataset).outerjoin(
            downloads_subquery, FoodDataset.id == downloads_subquery.c.dataset_id
        ).outerjoin(
            views_subquery, FoodDataset.id == views_subquery.c.dataset_id
        ).order_by(
